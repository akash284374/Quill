...
const Sidebar = () => {
  ...
  // While checking auth status, show placeholder
  if (loading) {
    return (
  ...
  // Fallback identifier (username → _id → "me")
  const profileId = user?.username || user?._id || "me";

  // Full sidebar for logged-in user
  return (
    <div className="w-64 h-screen bg-white text-gray-800 dark:bg-gray-900 dark:text-white flex flex-col justify-between px-6 py-8 shadow-lg border-r border-gray-200 dark:border-gray-700 transition-colors duration-300">
      {/* Top Section */}
...
// SidebarItem handles both "to" (Link) and "onClick" (button)
